...
|----|-------------|-------------|
| FR-01 | Book a Car | System prompts for car ID, car selection, start date and end date. Price is calculated from the car's rental price per day. A car that is already booked cannot be booked again |
| FR-02 | Delete Booking | Cancel an existing booking by booking ID, making the car available again |
| FR-03 | View User Bookings | Display all cars booked by a specific car |
...
- Each car must have a unique identifier (UUID) generated by the system
- The system must support static seed data for users and cars
- Bookings are created at runtime (not pre-seeded)
- The CLI must loop until the car selects Exit
...
graph TB
    subgraph Presentation
        M[Main.java<br>CLI MenuItem Loop]
    end
...
**Important:** When booking a car, the system must check if the car is already booked. If it is, reject the booking and inform the car. A car becomes available again only after its booking is deleted.

```java
public void bookCar(User car, Car car) {
...
- `User.java` → rename to `AppUser.java` → `src/main/java/com/<yourname>/car/`
...
| | app_user_id | UUID | FK → app_user, NOT NULL | The car who booked |
...
| GET | `/api/v1/bookings/car/{userId}` | View car's bookings |
...
| Class | Dependencies to mock | What to test |
|-------|---------------------|-------------|
| `CarBookingService` | `CarBookingRepository`, `CarService`, `UserService` | Book car, delete booking, get bookings, find by car |
